# @shapeshift-labs/frontier-lang-html

HTML semantic merge evidence and projection adapter for Frontier Lang semantic source documents.

This package is runtime-neutral. It emits deterministic HTML from Frontier Lang documents and builds source-bound HTML tree evidence for merge admission. Browser behavior, hydration, custom element lifecycle, framework directives, script execution, and style/layout effects remain explicit proof gaps unless a host supplies stronger evidence.
...
`sourceMap.mappings` links emitted element blocks back to Frontier Lang semantic node ids. `createHtmlSemanticMergeEvidence` records element identity, attributes, class lists, child/text/comment source spans, stable hashes, and fail-closed proof gaps for runtime-sensitive HTML surfaces. `safeMergeHtmlSource` admits independent text, attribute, token-level `class` / `part` / `itemprop` additions/removals, explicit-key element subtree, add-only and delete-only unkeyed child subtrees under an explicit parent with no sibling structural race, and explicit-key same-parent child-order edits when source spans, unique explicit element identity, and proof gaps are clean. Add-only unkeyed child evidence is reported as `htmlUnkeyedStructuralAddEvidence`; delete-only unkeyed child evidence is reported as `htmlUnkeyedStructuralDeleteEvidence`; token-list merges emit `htmlTokenListMergeEvidence`, and class merges also retain `htmlClassTokenMergeEvidence` for compatibility. Duplicate tokens and order-only token-list ambiguity fail closed. Parser evidence includes source-span, attribute-span, structural-span, and leading-trivia coverage counters so callers can distinguish full parser-backed coverage from partial evidence. Runtime-sensitive regions still block by default, but a host can attach an `html-source-bound-browser-runtime-proof` / `html-browser-runtime-proof` that is bound to the exact source path, reason, side, record key, base/worker/head/output source hashes, runtime command, probe id, evidence hash, and matching runtime signal to admit that specific change. Inline `on*` event-handler, `style`, iframe runtime attributes, iframe `srcdoc`, form submission attributes, submitter overrides, form-control state/value/validation attributes, document base/metadata attributes, resource-loading attributes, `<template>`, `<slot>`, custom elements, and framework directives use the same source-bound path with `html-source-bound-runtime-boundary-proof`, boundary metadata, changed attribute names, and boundary-specific runtime evidence signals where relevant.

## Support Boundary

- Ready evidence: element tree identity, attributes, classes, text nodes, comments, source spans, stable hashes, source-bound browser/runtime proofs with runtime command, probe id, evidence hash, and boundary-specific signal metadata.
- Auto-merge candidates: independent existing text-node edits, disjoint existing element-attribute edits, token-level `class` / `part` / `itemprop` additions/removals with duplicate/order-only token-list changes fail-closed, explicit `id` / `data-frontier-key` element subtree additions or deletions, add-only or delete-only unkeyed child subtrees under an explicit parent when no same-parent structural edit races on either side, same-parent reorders where all direct element siblings in the reordered range have explicit identity, and runtime-boundary edits only when an exact source-bound browser/runtime proof with runtime evidence metadata is supplied.
- Review-only gaps: duplicate explicit `id` / `data-frontier-key` identities, unkeyed structural moves/reorders or unkeyed adds/deletes under unkeyed parents or racing same-parent structural edits, reorders mixed with unkeyed direct siblings or direct comments, changed inline event-handler attributes, changed inline style attributes, changed iframe runtime attributes, changed iframe `srcdoc`, changed form submission attributes, changed submitter overrides, changed form-control state/value/validation attributes, changed document base/metadata attributes, changed resource-loading attributes, `<script>`, `<style>`, `<template>`, custom elements, slots, framework directives, hydration, browser DOM/runtime equivalence without exact source-bound proof.
- Claims: `autoMergeClaim`, `semanticEquivalenceClaim`, and broad render-equivalence claims remain false. `browserRuntimeEquivalenceClaim` is only true on a safe-merge result when a source-bound browser/runtime proof admits the specific runtime-boundary change with runtime evidence metadata.
